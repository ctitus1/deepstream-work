...
| `/ds/mode/toggle_detection` (`SetBool`) | `true`: auto-enqueue every `continuous.stride`-th frame (default 3 ≈ 10 Hz) onto the **continuous** queue and auto-run. `false`: stop the stream and discard *its own* not-yet-run frames (count in the response; manually enqueued frames are kept, and a run already in flight completes). Leaves the assessment setting untouched, so a restart resumes with whatever it was. Manual runs stay available throughout and take priority. |
| `/ds/mode/toggle_assessment` (`SetBool`) | `true`/`false`: open or close the assess valve on continuous runs. Independent of `toggle_detection` — it never starts or stops the stream, and never disturbs the stride cadence. Set while detection is off it is remembered and applies when detection next starts. |
